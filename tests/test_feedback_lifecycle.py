...
class TestSchemaMigration:
    ...
    def test_feedback_counters_round_trip(self) -> None:
        ...
        try:
            ...
            backend.store(entry)
            backend.update("m-2", recall_count=15, helpful_count=5, unhelpful_count=2)
            loaded = backend.get("m-2")
            assert loaded is not None
...
class TestFeedbackDecayScore:
    ...
    def test_1000_recalls_no_helpful_near_zero(self) -> None:
        """1000 recalls, 0 helpful: near zero."""
        score = feedback_decay_score(importance=0.8, recall_count=1000, helpful_count=0)
